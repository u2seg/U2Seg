...
The clustering categories could be set using `NUM_SELECTED_SAMPLES` in the config file `configs/ImageNet_usl_dino_0.2.yaml`. And the feature calculating batchsize could be set using `BATCH_SIZE`.  Path to dataset could be set using `ROOT_DIR`
