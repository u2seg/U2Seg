# Credit: https://github.com/Separius/SimCLRv2-Pytorch/blob/main/resnet.py
import torch
...
class Blocks(nn.Module):
    def __init__(self, num_blocks, in_channels, out_channels, stride, sk_ratio=0, checkpoint_segments=0):
        super().__init__()
    ...
    def forward(self, x):
        if self.checkpoint_segments:
            # Note: set preserve_rng_state to True for random layers such as Dropout
            return torch.utils.checkpoint.checkpoint_sequential(self.blocks, self.checkpoint_segments, x, preserve_rng_state=False)
...
class ResNet(nn.Module):
    def __init__(self, layers, width_multiplier, sk_ratio, checkpointing=False):
        super().__init__()
        ...
        ops.append(Blocks(layers[0], channels_in, 64 * width_multiplier, 1, sk_ratio, checkpoint_segments))
        channels_in = ops[-1].channels_out
        ops.append(Blocks(layers[1], channels_in, 128 * width_multiplier, 2, sk_ratio, checkpoint_segments))
        channels_in = ops[-1].channels_out
        ops.append(Blocks(layers[2], channels_in, 256 * width_multiplier, 2, sk_ratio, checkpoint_segments))
        channels_in = ops[-1].channels_out
        ops.append(Blocks(layers[3], channels_in, 512 * width_multiplier, 2, sk_ratio, checkpoint_segments))
        channels_in = ops[-1].channels_out
    ...
    def forward(self, h, apply_fc=False):
        # This is block-wise checkpointing, saves 30% memory on ResNet 50 2x with SK
        # if self.checkpointing:
        #     # First layer
        #     h = self.ops[0](h)
        #     # Subsequent layers
        #     h = torch.utils.checkpoint.checkpoint_sequential(self.ops[1:], 4, h, preserve_rng_state=False)
        # else:
        #     h = self.net(h)
        h = self.net(h)
        h = h.mean(dim=[2, 3])
...
def get_resnet(depth=50, width_multiplier=1, sk_ratio=0, num_layers=3, **kwargs):  # sk_ratio=0.0625 is recommended
    layers = {50: [3, 4, 6, 3], 101: [3, 4, 23, 3], 152: [3, 8, 36, 3], 200: [3, 24, 36, 3]}[depth]
    resnet = ResNet(layers, width_multiplier, sk_ratio, **kwargs)
    return resnet, ContrastiveHead(resnet.channels_out, num_layers=num_layers)
