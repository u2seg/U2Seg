...
@hydra.main(config_path="configs", config_name="batch_infer_config_cityscape.yml")
def my_app(cfg: DictConfig) -> None:
    ...
    for model_path in cfg.model_paths:
        ...
        save_dir = "/old_home_that_will_be_deleted_at_some_point/xinyang/CutLER/datasets/stego_cityscape_semantic_seg"
        ...
        with get_context('spawn').Pool(cfg.num_workers + 5) as pool:
            for i, batch in enumerate(tqdm(test_loader)):
                with torch.no_grad():
                    ...
                    # 保存 linear_preds 为 npy 文件
                    for idx, ind in enumerate(batch['ind']):
                        file_path = os.path.join(save_dir, f"{ind.cpu().item()}.npy")  # 注意这里对 ind 的处理
                        # 转换为 int32 类型并保存
                        np_array = linear_preds[idx].cpu().numpy().astype(np.int32)
