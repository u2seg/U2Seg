...
class CocoInference(Dataset):
    def __init__(self, root, image_list_path, transform=None, target_transform=None, image_set="val"):
        super(CocoInference, self).__init__()
    ...
    def __getitem__(self, index):
        # 加载图像
        image_path = join(self.image_root, self.image_files[index])
        img = Image.open(image_path).convert("RGB")

        if self.transform:
            img = self.transform(img)

        # 创建一个全零的标签作为占位符，大小与图像相同
        label = torch.zeros((img.size(1), img.size(2)), dtype=torch.long)
        # print('$'*20)
        # print(img)
        # print(img.shape)
        # print(label.shape)
...
class ImageNetInference(Dataset):
    def __init__(self, root, image_list_path, transform=None, target_transform=None, image_set="val"):
        super(ImageNetInference, self).__init__()
...
class ContrastiveSegDataset(Dataset):
    def __init__(self,
        ...
        if dataset_name == "potsdam":
            ...
            self.n_classes = 27
            dataset_class = CocoInference
            image_list_path = "/old_home_that_will_be_deleted_at_some_point/xinyang/CutLER/datasets/cityscape_img_list.txt" # 图片的文件列表路径
            extra_args = {
                'image_list_path': image_list_path
            }
        elif dataset_name == "imagenet":
            self.n_classes = 27
            dataset_class = ImageNetInference
            image_list_path = "/home/xinyang/Panoptic-CutLER/data/imagenet_list.txt"  # 图片的文件列表路径
            extra_args = {
                'image_list_path': image_list_path
            }
