...
feats_list = utils.get_feats_list(
    model, train_memory_loader, recompute=cfg.RECOMPUTE_Feat, CLIP=clip, DINO=DINO, force_no_extra_kwargs=True)
...
d_knns, ind_knns = utils.partitioned_kNN(
    feats_list, K=cfg.USL.KNN_K, recompute=cfg.RECOMPUTE_KNN)
...
for kMeans_seed in cfg.USL.SEEDS:
    ...
    cluster_labels, centroids = utils.run_kMeans(feats_list, num_centroids, final_sample_num, train_memory_dataset=train_memory_dataset, Niter=cfg.USL.K_MEANS_NITERS, recompute=recompute_num_dependent, seed=kMeans_seed, force_no_lazy_tensor=False)
