...
# cluster_labels, neighbors_dist, centroid_ordering, final_sample_num
def get_selection_without_reg_outliers(data, neighbors_dist, cluster_labels, num_centroids, iters=1, final_sample_num=None, w=1, momentum=0.5, horizon_num=256, alpha=1, exclude_same_cluster=False, verbose=False):
    ...
    cluster_labels_cuda = cluster_labels.cuda()
    neighbors_dist_cuda = neighbors_dist.cuda()
    if isinstance(num_centroids, int):
        # This is for compatibility: the argument is num_centroids and we use order from 0
        num_centroids = range(num_centroids)
    for cls_ind in tqdm(num_centroids):
        match_arr = cluster_labels_cuda == cls_ind
        if not torch.any(match_arr):
            continue
        # Select the instance with lowest distance:
...
def get_selection(selection_fn, *args, seed=None, recompute=True, save=True, pass_seed=False, **kwargs):
    ...
    # final_sample_num = kwargs.pop("final_sample_num", None)
    ...
    if recompute:
        if pass_seed:
            ...
            print("Keyword arguments:", argspec.kwonlyargs)
